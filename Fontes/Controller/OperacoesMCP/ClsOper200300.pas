...
interface
  uses ClsOperacoes,Xml.xmldom, Xml.XMLIntf, Xml.Win.msxmldom, Xml.XMLDoc,
       System.SysUtils, ServidorControl, ClsParametros, System.Classes, clsProva;
...
type
  ...
  public
    ...
    Procedure MontaXMLEnvioSinc(argXMLSinc: string);
...
begin
  ...
  // Aqui emplemeta a rotina de retorno
    ...
    begin
      GravaArquivoLog('Operação 200300 - Retorno Padrão - '+Erro.Message);
      LerXML.Active := False;
      LerXML.XML.Clear;
...
begin
  FUsuario := ParUsuario;
  if (Trim(FUsuario) <> '') and (Trim(FTipo) <> '') and (Trim(FAndamento) <> '') and (Fid_prova <> 0) then
    MontaXMLEnvio(self);
end;
...
//  XMLDoc.SaveToFile(UserProgranData + '\Criar\e-Prova XE\enviar\200300' + IntToStr(ParQuestionario.Id_prova) + ParCandidato.CPFCandidato + '.ep');
...
Procedure Operacao200300.MontaXMLEnvioSinc(argXMLSinc: string);
var
  XMLDoc: IXMLDocument;
  XMLElementoEnvio: IXMLNode;
  XMLExame: IXMLNode;
...
begin
  ...
  XMLDoc := TXMLDocument.Create(nil);
  XMLDoc.Active := False;
  XMLDoc.LoadFromFile(argXMLSinc);
  XMLDoc.Active := True;

  XMLDoc.SaveToXML( XmlTexto );

  Operacao := MainOperacao.Create(Self, '200300', ' <?xml version="1.0" encoding="UTF-8"?> ' + XmlTexto, onMontaXMLRetorno);
  Operacao.consutar;
