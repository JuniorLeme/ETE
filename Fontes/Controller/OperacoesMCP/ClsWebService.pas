...
type
  ...
  Main = interface(IInvokable)
    ...
    function cripto_execute(const program_server: string; const token: string;
      const xml: string): string; stdcall;
    function secure_execute(const program_server: string; const token: string;
      const xml: string): string; stdcall;
    function token(const program_server: string; const program_version: string;
      const user: string; const password: string): string; stdcall;
    function validate(const program_server: string; const token: string)
      : string; stdcall;
    function execute(const idserver: string; const xml: string)
      : string; stdcall;
  end;

function GetMain(UseWSDL: boolean = System.False; Addr: string = '';
  HTTPRIO: THTTPRIO = nil): Main;
...
procedure BadPaddingException2.Setmessage_(Index: Integer;
...
procedure BadPaddingException.Setmessage_(Index: Integer;
  const Astring: string);
begin
  Fmessage_ := Astring;
  Fmessage__Specified := True;
...
procedure IllegalBlockSizeException2.Setmessage_(Index: Integer;
  const Astring: string);
begin
  Fmessage_ := Astring;
  Fmessage__Specified := True;
...
procedure IllegalBlockSizeException.Setmessage_(Index: Integer;
  const Astring: string);
begin
  Fmessage_ := Astring;
  Fmessage__Specified := True;
...
{ Main }
InvRegistry.RegisterInterface(TypeInfo(Main), 'http://mcp.grupocriar/',
  'UTF-8');
InvRegistry.RegisterDefaultSOAPAction(TypeInfo(Main), '');
InvRegistry.RegisterInvokeOptions(TypeInfo(Main), ioDocument);
{ Main.cripto_execute }
InvRegistry.RegisterMethodInfo(TypeInfo(Main), 'cripto_execute', '',
  '[ReturnName="return"]', IS_OPTN or IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'cripto_execute',
  'program_server', '', '', IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'cripto_execute', 'token', '',
  '', IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'cripto_execute', 'xml', '',
  '', IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'cripto_execute', 'return', '',
  '', IS_UNQL);
{ Main.secure_execute }
InvRegistry.RegisterMethodInfo(TypeInfo(Main), 'secure_execute', '',
  '[ReturnName="return"]', IS_OPTN or IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'secure_execute',
  'program_server', '', '', IS_UNQL);
...
InvRegistry.RegisterMethodInfo(TypeInfo(Main), 'execute', '',
  '[ReturnName="return"]', IS_OPTN or IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'execute', 'idserver', '',
  '', IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'execute', 'xml', '', '',
  IS_UNQL);
InvRegistry.RegisterParamInfo(TypeInfo(Main), 'execute', 'return', '',
  '', IS_UNQL);
RemClassRegistry.RegisterXSClass(BadPaddingException2, 'http://mcp.grupocriar/',
  'BadPaddingException2', 'BadPaddingException');
RemClassRegistry.RegisterExternalPropName(TypeInfo(BadPaddingException2),
  'message_', '[ExtName="message"]');
RemClassRegistry.RegisterXSClass(BadPaddingException, 'http://mcp.grupocriar/',
  'BadPaddingException');
RemClassRegistry.RegisterExternalPropName(TypeInfo(BadPaddingException),
  'message_', '[ExtName="message"]');
RemClassRegistry.RegisterXSClass(IllegalBlockSizeException2,
  'http://mcp.grupocriar/', 'IllegalBlockSizeException2',
  'IllegalBlockSizeException');
RemClassRegistry.RegisterExternalPropName(TypeInfo(IllegalBlockSizeException2),
  'message_', '[ExtName="message"]');
RemClassRegistry.RegisterXSClass(IllegalBlockSizeException,
  'http://mcp.grupocriar/', 'IllegalBlockSizeException');
RemClassRegistry.RegisterExternalPropName(TypeInfo(IllegalBlockSizeException),
  'message_', '[ExtName="message"]');
