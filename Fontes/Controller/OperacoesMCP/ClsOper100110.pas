        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if NoServidores.ChildNodes.Get(S).ChildNodes.Get(E) <> nil then
                ListaHostNames.Add(NoServidores.ChildNodes.Get(S)
                  .ChildNodes.Get(E).Text);

            end;

            Parametros.ListaServidores.AddObject(NoServidores.ChildNodes.Get(S)
              .Attributes['id'], ListaHostNames);
          ...
          begin
            ...
            for T := 0 to Parametros.ListaOperacoesTimeOut.Count - 1 do
              Parametros.ListaOperacoesTimeOut[T] := NoTempos.Attributes
                ['padrao'];

            for T := 0 to NoTempos.ChildNodes.Count - 1 do
            begin

              if Parametros.ListaOperacoes.IndexOf(NoTempos.ChildNodes.Get(T)
                .Attributes['operacao']) > -1 then
              begin
                Parametros.ListaOperacoesTimeOut
                  [Parametros.ListaOperacoes.IndexOf(NoTempos.ChildNodes.Get(T)
                  .Attributes['operacao'])] := NoTempos.ChildNodes.Get(T).Text;
              end;
