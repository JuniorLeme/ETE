...
interface
  uses ClsOperacoes,Xml.xmldom, Xml.XMLIntf, Xml.Win.msxmldom, Xml.XMLDoc,
    System.SysUtils, System.Classes, ClsParametros, ClsProva;
...
begin
  try
    ...
    try
      ...
      begin
        GravaArquivoLog(self,'Operação 200600 - Retorno Padrão -' + Erro.Message);
        LerXML.Active := False;
        LerXML.Xml.Clear;
    ...
    on E: Exception do
    begin
      GravaArquivoLog(self,'Operação 200600 - ' + E.Message);
      LerXML.Active := False;
      LerXML.Xml.Clear;
      LerXML.Xml.Text := Operacao.XMLRetornoPadrao;
      LerXML.Active := True;
