...
type
  ...
  public
    ...
    function token(Arg: TJSONObject; const ARequestFilter: string = '')
      : TJSONObject;
    function token_Cache(Arg: TJSONObject; const ARequestFilter: string = '')
      : IDSRestCachedJSONObject;
    function updatetoken(Arg: TJSONObject; const ARequestFilter: string = '')
      : TJSONObject;
    function updatetoken_Cache(Arg: TJSONObject;
      const ARequestFilter: string = ''): IDSRestCachedJSONObject;
    function biometria_foto(Arg: TJSONObject; const ARequestFilter: string = '')
      : TJSONObject;
    function biometria_foto_Cache(Arg: TJSONObject;
      const ARequestFilter: string = ''): IDSRestCachedJSONObject;
    function updatebiometria_foto(Arg: TJSONObject;
      const ARequestFilter: string = ''): TJSONObject;
    function updatebiometria_foto_Cache(Arg: TJSONObject;
      const ARequestFilter: string = ''): IDSRestCachedJSONObject;
  end;

const
  operacao_token: array [0 .. 1] of TDSRestParameterMetaData = ((Name: 'Arg';
    Direction: 1; DBXType: 37; TypeName: 'TJSONObject'), (Name: '';
  ...
  operacao_updatebiometria_foto_Cache: array [0 .. 1]
    of TDSRestParameterMetaData = ((Name: 'Arg'; Direction: 1; DBXType: 37;
    TypeName: 'TJSONObject'), (Name: ''; Direction: 4; DBXType: 26;
    TypeName: 'String'));

implementation

function TOperClientBiometria.token(Arg: TJSONObject;
  const ARequestFilter: string): TJSONObject;
begin
  ...
  Result := TJSONObject(FtokenCommand.Parameters[1].Value.GetJSONValue
    (FInstanceOwner));
end;

function TOperClientBiometria.token_Cache(Arg: TJSONObject;
  const ARequestFilter: string): IDSRestCachedJSONObject;
begin
  ...
  Result := TDSRestCachedJSONObject.Create(FtokenCommand_Cache.Parameters[1]
    .Value.GetString);
end;

function TOperClientBiometria.updatetoken(Arg: TJSONObject;
  const ARequestFilter: string): TJSONObject;
begin
  ...
  Result := TJSONObject(FupdatetokenCommand.Parameters[1].Value.GetJSONValue
    (FInstanceOwner));
end;

function TOperClientBiometria.updatetoken_Cache(Arg: TJSONObject;
  const ARequestFilter: string): IDSRestCachedJSONObject;
begin
  ...
  FupdatetokenCommand_Cache.Parameters[0].Value.SetJSONValue(Arg,
    FInstanceOwner);
  FupdatetokenCommand_Cache.ExecuteCache(ARequestFilter);
  Result := TDSRestCachedJSONObject.Create(FupdatetokenCommand_Cache.Parameters
    [1].Value.GetString);
end;

function TOperClientBiometria.biometria_foto(Arg: TJSONObject;
  const ARequestFilter: string): TJSONObject;
begin
  ...
  Result := TJSONObject(Fbiometria_fotoCommand.Parameters[1].Value.GetJSONValue
    (FInstanceOwner));
end;

function TOperClientBiometria.biometria_foto_Cache(Arg: TJSONObject;
  const ARequestFilter: string): IDSRestCachedJSONObject;
begin
  ...
  Fbiometria_fotoCommand_Cache.Parameters[0].Value.SetJSONValue(Arg,
    FInstanceOwner);
  Fbiometria_fotoCommand_Cache.ExecuteCache(ARequestFilter);
  Result := TDSRestCachedJSONObject.Create
    (Fbiometria_fotoCommand_Cache.Parameters[1].Value.GetString);
end;

function TOperClientBiometria.updatebiometria_foto(Arg: TJSONObject;
  const ARequestFilter: string): TJSONObject;
begin
  ...
  Fupdatebiometria_fotoCommand.Parameters[0].Value.SetJSONValue(Arg,
    FInstanceOwner);
  Fupdatebiometria_fotoCommand.Execute(ARequestFilter);
  Result := TJSONObject(Fupdatebiometria_fotoCommand.Parameters[1]
    .Value.GetJSONValue(FInstanceOwner));
end;

function TOperClientBiometria.updatebiometria_foto_Cache(Arg: TJSONObject;
  const ARequestFilter: string): IDSRestCachedJSONObject;
begin
  ...
  begin
    ...
    Fupdatebiometria_fotoCommand_Cache.Text :=
      'operacao."updatebiometria_foto"';
    Fupdatebiometria_fotoCommand_Cache.Prepare
      (operacao_updatebiometria_foto_Cache);
  end;
  Fupdatebiometria_fotoCommand_Cache.Parameters[0].Value.SetJSONValue(Arg,
    FInstanceOwner);
  Fupdatebiometria_fotoCommand_Cache.ExecuteCache(ARequestFilter);
  Result := TDSRestCachedJSONObject.Create
    (Fupdatebiometria_fotoCommand_Cache.Parameters[1].Value.GetString);
end;
